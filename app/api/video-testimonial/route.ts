...
export async function POST(request: NextRequest) {
  try {
    console.log('📹 Video testimonial upload started')
    ...
    if (reviewLinkId) {
      ...
      if (linkData && !reviewLinkError) {
        ...
        console.log('📧 Found business owner email:', userEmail)
      } else {
        console.warn('⚠️ Could not find business owner email for review link:', reviewLinkId)
      }
    }
    ...
    try {
      ...
      if (dbError) {
        ...
        console.log('💡 Please create the video_testimonials table manually in Supabase')
      ...
      console.log('💡 Video upload will continue without database record')
    }
    ...
    if (userEmail) {
      ...
      setImmediate(async () => {
        try {
          ...
          console.log('✅ Email sent successfully with video attachment')
      ...
      console.log('📧 Email queued for background sending')
    } else {
      console.log('⚠️ No business owner email found - skipping email notification')
    }
